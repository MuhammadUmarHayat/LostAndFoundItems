...
def home():
    ...
    matched = find_found_lost_images()
   # print(matched)
    user_notifications = get_user_notifications(matched, current_user)
...
@customer_app.route("/allnotifications")
def allnotifications():
    ...
    db = get_db()
    cursor = db.cursor()
    cursor.execute("SELECT * FROM founds")
    items = cursor.fetchall()
...
#     user_notifications = user_notifications[::-1]
...
#     end = start + per_page
#     notifications_paginated = user_notifications[start:end]
...
from PIL import Image
import imagehash
#find lost images for notifications
def find_found_lost_images( ):
    """
    Returns lost images that are found (matched)
    """
    lost_folder=f"static/lostItems"
    found_folder=f"static/foundItems"
    threshold=5
    found_lost_images = []  # container to return

    # Step 1: Create hash map for lost images
    lost_hashes = {}

    for lost_img in os.listdir(lost_folder):
        lost_path = os.path.join(lost_folder, lost_img)

        if lost_img.lower().endswith(('.jpg', '.jpeg', '.png')):
            img = Image.open(lost_path)
            lost_hashes[lost_img] = imagehash.phash(img)
            # print("Lost inages :"+img)

    # Step 2: Compare each found image with lost images
    for found_img in os.listdir(found_folder):
        found_path = os.path.join(found_folder, found_img)

        if found_img.lower().endswith(('.jpg', '.jpeg', '.png')):
            found_image = Image.open(found_path)
            found_hash = imagehash.phash(found_image)
            # print("found inages :"+img)

            for lost_img, lost_hash in lost_hashes.items():
                diff = found_hash - lost_hash

                if diff <= threshold:
                    found_lost_images.append({
                        "lost_image": lost_img,
                        "found_image": found_img,
                        "similarity_score": diff
                    })

    return found_lost_images

def get_user_notifications(matched_images, current_user):
    """
    Returns matched lost images ONLY for logged-in user
    """
    notifications = []

    for match in matched_images:
        lost_image_name = match["lost_image"]
        lost_image_path = os.path.join("static/lostItems", lost_image_name)

        meta = readLostItem(lost_image_path)

        if str(meta["user_id"]) == str(current_user):
            notifications.append({
                "lost_image": lost_image_name,
